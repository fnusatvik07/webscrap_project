...
def scrape_and_store(asin,geo_location,domain):
...
def fetch_and_store_competitors(parent_asin,domain,geo_location,pages=2):
    ...
    if parent.get("categories"):
        search_categories.extend(str(cat) for cat in parent["categories"])
    
    if parent.get("category_path"):
        search_categories.extend(str(cat) for cat in parent["category_path"])
    ...
    product_details=scrape_multiple_products(competitor_asins[:20],search_domain,search_geo)
